							...
							while($info_row=$info_stmt->fetch(PDO::FETCH_ASSOC)){
								...
								if($t_like_chk < 1){	
									...
									$tl_chk_query = "
										SELECT *
										FROM TB_TOTAL_LIKE
										WHERE con_Idx = :con_Idx AND place_Idx = :place_Idx
									";
									$tl_chk_stmt = $DB_con->prepare($tl_chk_query);
									$tl_chk_stmt->bindParam(":con_Idx", $con_Idx);
									$tl_chk_stmt->bindParam(":place_Idx", $place_Idx);
									$tl_chk_stmt->execute();
									$tl_chk_num = $tl_chk_stmt->rowCount();
									if($tl_chk_num > 0 ){
