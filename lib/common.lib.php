...
function set_cookie($cookie_name, $value, $expire)
{
    ...
    setcookie(md5($cookie_name), base64_encode($value), DU_SERVER_TIME + $expire, '/', DU_COOKIE_DOMAIN);
}
...
function get_list($write_row, $board, $skin_url, $subject_len=40)
{
    ...
    $list['icon_new'] = '';
    if ($board['bo_new'] && $list['wr_datetime'] >= date("Y-m-d H:i:s", DU_SERVER_TIME - ($board['bo_new'] * 3600)))
...
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    ...
    if($config['cf_point_term'] > 0) {
        if($expire > 0)
            $po_expire_date = date('Y-m-d', strtotime('+'.($expire - 1).' days', DU_SERVER_TIME));
        else
            $po_expire_date = date('Y-m-d', strtotime('+'.($config['cf_point_term'] - 1).' days', DU_SERVER_TIME));
    }
...
{
    $str = call_user_func(DU_ESCAPE_FUNCTION, $field);
    return $str;
}
...
function is_mobile()
{
    return preg_match('/'.DU_MOBILE_AGENT.'/i', $_SERVER['HTTP_USER_AGENT']);
}
...
class html_process {
    ...
    function run()
    {
        ...
        if ($tmp_row['cnt']) {
            ...
            // 시간이 지난 접속은 삭제한다
            sql_query(" delete from {$g5['login_table']} where lo_datetime < '".date("Y-m-d H:i:s", DU_SERVER_TIME - (60 * $config['cf_login_minutes']))."' ");
...
function member_delete($mb_id)
{
    ...
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_birth = '', mb_sex = '', mb_signature = '', mb_memo = '".date('Ymd', DU_SERVER_TIME)." 삭제함\n{$mb['mb_memo']}' where mb_id = '{$mb_id}' ";
    sql_query($sql);
...
// mysql_real_escape_string 사용법|작성자 이종성
// ex) quote_smart( $_POST[ 'password' ] ) );
