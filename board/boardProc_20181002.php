...
*/
		   ...
		   $b_Ip = escape_trim($_SERVER['REMOTE_ADDR']);
			...
			$insQuery = "INSERT INTO TB_BOARD (b_Idx, b_NIdx, b_Cate, b_MemId, b_Title, b_Name, b_Content, b_RContent, b_Ref, b_RefStep, b_RefOrd, b_Not, b_Hide, b_Ip, reg_Date) VALUES (:b_Idx, :b_NIdx, :b_Cate, :b_MemId, :b_Title, :b_Name, :b_Content, :b_RContent, :b_Ref, :b_RefStep, :b_RefOrd, :b_Not, :b_Hide, :b_Ip, :reg_Date)";
			//echo $insQuery."<BR>";
			//exit;
...
$sql = "UPDATE TB_BOARD SET b_Title = :b_Title, 
            b_Content = :b_Content, 
            b_Name = :b_Name
            WHERE b_Idx = :b_Idx
			AND b_NIdx = :b_NIdx"
...
$stmt->bindParam(':b_Content', $b_Content, PDO::PARAM_STR);    
$stmt->bindParam(':b_Name', $b_Name, PDO::PARAM_STR);
// use PARAM_STR although a number  
$stmt->bindParam(':b_Idx', $b_Idx, PDO::PARAM_INT); 
$stmt->bindParam(':b_NIdx', $idx, PDO::PARAM_INT);   
$stmt->execute(); 


*/

			$upQuery = "UPDATE TB_BOARD SET b_Cate = :b_Cate, b_Title = :b_Title, b_Name = :b_Name, b_Content = :b_Content, b_RContent = :b_RContent, b_Not = :b_Not, b_Hide = :b_Hide WHERE b_Idx = :b_Idx AND b_NIdx = :b_NIdx LIMIT 1";
			//$upQuery = "UPDATE TB_BOARD SET b_Cate = $b_Cate, b_Title = $b_Title, b_Name = $b_Name, b_Content = $b_Content, b_RContent = $b_RContent, b_Not = $b_Not, b_Hide = $b_Hide WHERE b_Idx = $b_Idx AND b_NIdx = $idx LIMIT 1";

//echo $upQuery."<BR>";

			$upStmt = $DB_con->prepare($upQuery);
			$upStmt->bindparam(":b_Cate",$b_Cate, PDO::PARAM_STR);
			$upStmt->bindparam(":b_Title",$b_Title, PDO::PARAM_STR);
			$upStmt->bindparam(":b_Name",$b_Name, PDO::PARAM_STR);
			$upStmt->bindparam(":b_Content",$b_Content, PDO::PARAM_STR);
			$upStmt->bindparam(":b_RContent",$b_RContent, PDO::PARAM_STR);
			$upStmt->bindparam(":b_Not",$b_Not, PDO::PARAM_STR);
			$upStmt->bindparam(":b_Hide",$b_Hide, PDO::PARAM_STR);
			$upStmt->bindparam(":b_Idx",$b_Idx);
			$upStmt->bindparam(":b_NIdx",$idx);
			$upStmt->execute();

			if($upStmt->rowCount() > 0 ) { //삽입 성공
				$preUrl = "../board/boardList.php?board_id=".$b_Idx;
				$message = "reg";
				proc_msg($message, $preUrl);
			} else {
			...
			if($foldNum < 1)  { //아닐경우
				...
				while($foldRow=$deStmt->fetch(PDO::FETCH_ASSOC)) {
			...
			if($bFileNum < 1)  { //아닐경우
				...
				$delFileQuery = "DELETE FROM TB_BOARD_FILE WHERE  b_idx = :b_idx AND b_NIdx = :b_NIdx AND b_FIdx = :bFidx LIMIT 1";
