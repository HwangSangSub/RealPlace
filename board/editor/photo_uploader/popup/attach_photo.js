	...
	var nTotalSize = 0;
	var nMaxImageSize = 10*1024*1024; //하나에 10메가
	var nMaxTotalImageSize = 100*1024*1024; //  총 100메가 (php.ini 100메가까지 허용)
	var nMaxImageCount = 30;
    ...
    function drop(ev) {
		...
		if (nImageFileCount >= 30){
			alert("최대 30장까지만 등록할 수 있습니다.");
			return;
		}
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			alert("전체 이미지 용량이 100MB를 초과하여 등록할 수 없습니다. \n\n (파일명 : "+sFileName+", 사이즈 : "+sFileSize+")");
		} else {
			//이미지 정보 저장							
