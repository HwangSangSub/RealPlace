...
if(isset($_FILES['upload'])){ 
	...
	//		return $contents_json['searchPoiInfo']['pois']['poi']['0'];
		...
		if($chklistnum < 1){
			...
			$mRow1 = round((int)$maxRow / 2);
			$mRow2 = (int)$maxRow - (int)$mRow1;

			for ($i1 = 1 ; $i1 <= $mRow1; $i1++) {
				if($i1 == 1){
					continue;	
				}
				$e_Date = $objWorksheet->getCell('A' . $i1)->getValue();
				$e_Time = $objWorksheet->getCell('B' . $i1)->getValue();
				$e_State = $objWorksheet->getCell('C' . $i1)->getValue();
				$Manager = $objWorksheet->getCell('D' . $i1)->getValue();
				$p_Date = $objWorksheet->getCell('E' . $i1)->getValue();
				$p_Manager = $objWorksheet->getCell('F' . $i1)->getValue();
				$p_Time = $objWorksheet->getCell('G' . $i1)->getValue();
				$c_No = $objWorksheet->getCell('H' . $i1)->getValue();
				$c_Name = $objWorksheet->getCell('I' . $i1)->getValue();
				$car_No = $objWorksheet->getCell('J' . $i1)->getValue();
				$p_Name = $objWorksheet->getCell('K' . $i1)->getValue();
				$b_No = $objWorksheet->getCell('L' . $i1)->getValue();
				$d_Distance = $objWorksheet->getCell('M' . $i1)->getValue();
				$c_Distance = $objWorksheet->getCell('N' . $i1)->getValue();
				$r_Date = $objWorksheet->getCell('O' . $i1)->getValue();
				$rDate = PHPExcel_Style_NumberFormat::toFormattedString($r_Date, 'YYYY-MM-DD'); // 날짜 형태의 셀을 읽을때는 toFormattedString를 사용한다.
				$p_Tel = $objWorksheet->getCell('P' . $i1)->getValue();
				$h_Tel = $objWorksheet->getCell('Q' . $i1)->getValue();
				$z_Code = $objWorksheet->getCell('R' . $i1)->getValue();
				$n_Addr = $objWorksheet->getCell('S' . $i1)->getValue();
				$p_Addr = $objWorksheet->getCell('T' . $i1)->getValue();
				$d_Addr = $objWorksheet->getCell('U' . $i1)->getValue();
				$o_State = $objWorksheet->getCell('V' . $i1)->getValue();
				if($e_State == "초기배정"){
					$e_State = "0";
				}else{
					$e_State = "1";	//점검완료
			  ...
			  if($i1 > $mRow1){
				for ($i2 = $i1 ; $i2 <= $maxRow; $i2++) {
					$i3 = $i2;
					$e_Date = $objWorksheet->getCell('A' . $i2)->getValue();
					$e_Time = $objWorksheet->getCell('B' . $i2)->getValue();
					$e_State = $objWorksheet->getCell('C' . $i2)->getValue();
					$Manager = $objWorksheet->getCell('D' . $i2)->getValue();
					$p_Date = $objWorksheet->getCell('E' . $i2)->getValue();
					$p_Manager = $objWorksheet->getCell('F' . $i2)->getValue();
					$p_Time = $objWorksheet->getCell('G' . $i2)->getValue();
					$c_No = $objWorksheet->getCell('H' . $i2)->getValue();
					$c_Name = $objWorksheet->getCell('I' . $i2)->getValue();
					$car_No = $objWorksheet->getCell('J' . $i2)->getValue();
					$p_Name = $objWorksheet->getCell('K' . $i2)->getValue();
					$b_No = $objWorksheet->getCell('L' . $i2)->getValue();
					$d_Distance = $objWorksheet->getCell('M' . $i2)->getValue();
					$c_Distance = $objWorksheet->getCell('N' . $i2)->getValue();
					$r_Date = $objWorksheet->getCell('O' . $i2)->getValue();
					$rDate = PHPExcel_Style_NumberFormat::toFormattedString($r_Date, 'YYYY-MM-DD'); // 날짜 형태의 셀을 읽을때는 toFormattedString를 사용한다.
					$p_Tel = $objWorksheet->getCell('P' . $i2)->getValue();
					$h_Tel = $objWorksheet->getCell('Q' . $i2)->getValue();
					$z_Code = $objWorksheet->getCell('R' . $i2)->getValue();
					$n_Addr = $objWorksheet->getCell('S' . $i2)->getValue();
					$p_Addr = $objWorksheet->getCell('T' . $i2)->getValue();
					$d_Addr = $objWorksheet->getCell('U' . $i2)->getValue();
					$o_State = $objWorksheet->getCell('V' . $i2)->getValue();
					if($e_State == "초기배정"){
						$e_State = "0";
					}else{
						$e_State = "1";	//점검완료
				...
				$result = array("result" => "success", "mRow1" => $mRow1, "mRow2" => $mRow2, "maxRow" => $maxRow, "i1" => $i1, "i2" => $i2, "i3" => $i3);
		}else{
