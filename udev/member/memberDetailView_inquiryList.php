...
<div id="wrapper">
    <div id="container" class="">
        ...
        <h1 id="container_title"><?= $id?>(<?=$sqlRow['mem_NickNm']?>)&nbsp;회원상세보기 - 문의리스트 관리</h1>
		...
		<div class="local_ov01 local_ov">
			<span class="btn_ov01">
				...
				<span class="ov_txt"><a href="memberDetailView_inquiryList.php?id=<?=$id?>">문의리스트</a></span>
			</span>
		</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg?>">
		...
		<td><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap; ">
			<?if($b_State == '0'){?>
				<a href="/udev/etc/inquiryReg.php?mode=reg&idx=<?=$row['idx']?>&<?=$qstr?>&page=<?=$page?>"><span><?=	str_replace("?","",trim($row['b_Content'])) ?></span></a>
			<?}else{?>
				<a href="/udev/etc/inquiryReg.php?mode=mod&idx=<?=$row['idx']?>&<?=$qstr?>&page=<?=$page?>"><span><?=str_replace("?","",trim($row['b_Content'])) ?></span></a>
			<?}?></div></td>
		<td><?=$row['reg_Date']?></td>
		<td><?=$bState?></td>
...
<? if($_COOKIE['du_udev']['id'] != 'admin2'){?>
				<?if($b_State == '0'){?>
					<a href="/udev/etc/inquiryReg.php?mode=reg&idx=<?=$row['idx']?>&<?=$qstr?>&page=<?=$page?>" class="btn btn_03">답변등록</a>
				<?}else{?>
					<a href="/udev/etc/inquiryReg.php?mode=mod&idx=<?=$row['idx']?>&<?=$qstr?>&page=<?=$page?>" class="btn btn_02">답변수정</a>
				<?}?>
<? } ?>
		</td>
    </tr>
