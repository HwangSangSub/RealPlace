...
if(isset($_FILES['kmlfile'])){ 
	...
	if(move_uploaded_file($_FILES['kmlfile']['tmp_name'],$target)) {
		...
		if ($reg_Id != "" && $con_Name != "" && $category != ""  ) {
			...
			//회원 기본테이블 저장
			$query = "INSERT INTO TB_CONTENTS (con_Name, category, img, tag, open_Bit, kml_File, reg_Id, reg_date ) VALUES (:con_Name, :category, :img, :tag, :open_Bit, :kml_File, :reg_Id, :reg_Date)";
	...
	if ($reg_Id != "" && $con_Name != "" && $category != ""  ) {
		...
		//회원 기본테이블 저장
		$query = "INSERT INTO TB_CONTENTS (con_Name, category, img, tag, open_Bit, reg_Id, reg_date ) VALUES (:con_Name, :category, :img, :tag, :open_Bit, :reg_Id, :reg_Date)";
