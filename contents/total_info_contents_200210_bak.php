...
if ($con_Idx != "") {
	...
	if($area_Cnt != "0"){
		...
		// 장소정보
		$query = "
			SELECT idx, area_Code, place_Name, place_Icon, lng, lat, category, like_Cnt, coupon_Idx, reserv_Bit, coupon_Bit
			FROM TB_PLACE 
			WHERE con_Idx = :con_Idx
	...
	if($num == 1){
		...
		if($kml_File != ""){
			...
			for($nm = 0; $nm < $a_Cnt; $nm++){
				...
				// 좋아요가 많은 장소
				$like_query = "
					SELECT idx, place_Name, place_Icon, lng, lat, category, like_Cnt, like_Cnt, coupon_Idx, reserv_Bit, coupon_Bit
					FROM TB_PLACE 
					WHERE (".$lng_min." < lng AND lng < ".$lng_max.")
						AND (".$lat_min." < lat AND lat < ".$lng_max.")
					ORDER BY like_Cnt DESC, mod_Date DESC, reg_Date DESC LIMIT 5;
					";
