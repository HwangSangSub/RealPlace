...
if($_FILES['kmlfile']['name'] != ''){ 
	...
	if(move_uploaded_file($_FILES['kmlfile']['tmp_name'],$target)) {
		...
		if ($reg_Id != "" && $con_Name != "" && $category != ""  ) {
			...
			//콘텐츠등록
			$query = "INSERT INTO TB_CONTENTS (member_Idx, con_Name, con_Lv, category, img, tag, open_Bit, thumbnail_Bit, kml_File, reg_Id, reg_date, mod_Date ) VALUES (:member_Idx, :con_Name, :con_Lv, :category, :img, :tag, :open_Bit, :thumbnail_Bit, :kml_File, :reg_Id, :reg_Date, NOW())";
	...
	if ($reg_Id != "" && $con_Name != "" && $category != ""  ) {
		...
		//회원 기본테이블 저장
		$query = "INSERT INTO TB_CONTENTS (member_Idx, con_Name, con_Lv, category, img, tag, open_Bit, thumbnail_Bit, memo, reg_Id, reg_date, mod_Date ) VALUES (:member_Idx, :con_Name, :con_Lv, :category, :img, :tag, :open_Bit, :thumbnail_Bit, :memo, :reg_Id, :reg_Date, NOW())";
