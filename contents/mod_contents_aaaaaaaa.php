...
// 지도구독 삭제
	...
	*/
			...
			$result = array("result" => "success", "errorMsg" => "지도삭제완료");
		}
	}else{
		...
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
			...
			if($img == ""){
				...
				if($org_img != ""){
					...
					$img = $img_txt;	
					$mod_query = "UPDATE TB_CONTENTS SET category = :category,con_Name = :con_Name,img = :img,memo = :memo,open_Bit = :open_Bit,tag = :tag,mod_Date = NOW() WHERE member_Idx = :member_Idx AND idx = :con_Idx LIMIT 1;";
				...
				/*
				$mod_chk_query = "UPDATE TB_CONTENTS SET category = '".$category."', con_Name = '".$con_Name."', img = '".$img."', memo = '".$memo."', open_Bit = '".$open_Bit."', tag = '".$tag."', mod_Date = NOW() WHERE member_Idx = ".$member_Idx." AND idx = ".$con_Idx." LIMIT 1;";
				*/
				$mod_query = "UPDATE TB_CONTENTS SET category = :category,con_Name = :con_Name,img = :img,memo = :memo,open_Bit = :open_Bit,tag = :tag,mod_Date = NOW() WHERE member_Idx = :member_Idx AND idx = :con_Idx LIMIT 1;";
				$mod_stmt = $DB_con->prepare($mod_query);
				$mod_stmt->bindParam(":category", $category);
