...
if ($place_chk_num > 0) {
	...
	while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		...
		if($comment_num < 1){
			$comment_Cnt = "0";										// 지점댓글수
			$comment = array("c_mem_Id" => "", "c_member_Img" => "", "c_Idx" => "", "comment" => "", "c_reg_Date" => "");
		}else{
			$comment_chk = [];
	...
	$result = array("result" => "success", "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img, "place_Name" => $place_Name, "memo" => $memo, "tel" => $tel, "day_Bit" => $day_Bit, "otime_Day" => $otime_Day, "week_Bit" => $week_Bit, "otime_Week" => $otime_Week, "img_Cnt" => (string)$img_Cnt, "img" => $img, "like_Cnt" => (string)$like_Cnt, "share_Cnt" => (string)$share_Cnt, "comment_Cnt" => (string)$comment_Cnt, "comment_Add" => $comment_add, "comment" => $comment, "addr" => $addr, "lng" => $lng, "lat" => $lat, "reg_Id" => $reg_Id, "member_Img" => $member_Img, "reg_date" => $reg_date);
    dbClose($DB_con);
    $stmt = null;
} else { //빈값일 경우
