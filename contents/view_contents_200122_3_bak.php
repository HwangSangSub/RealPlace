...
if ($category != "") {
	...
	if($search != ""){	//검색어가 있을 경우 검색값 추가  
		...
		$cntQuery = "
			...
			WHERE ".$category."
				AND (a.con_Name like '%".$search."%' OR a.tag like '%".$search."%' OR a.memo like '%".$search."%')
            GROUP BY a.idx
	...
	if ($chkResult  == "1") {
		...
		echo  urldecode($output);  */
		$result = array("result" => "error", "lists_info" => $listInfoResult, "errorMsg" => "자료가 없습니다.");
