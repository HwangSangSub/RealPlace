...
if($chk_Cnt > 0){					//지점이 있는 경우
	if($mode == "0"){				//삭제처리
		$p_query = "
			SELECT idx, con_Idx, member_Idx, org_place_Idx, img, reg_Id
			FROM TB_PLACE
			WHERE idx = :place_Idx
		";
		...
		$org_place_Idx = $p_row['org_place_Idx'];	// 담은지점고유번호
		...
		// 3. 담기 내역 삭제
		if($org_place_Idx != ""){									// 담긴지점의 경우는 담은 지점의 공유 수 -1, 공유내역 삭제
			$chk_p_query = "
			...
			$chk_p_stmt->bindparam(":idx",$org_place_Idx);
			$chk_p_stmt->execute();
			$chk_p_row=$chk_p_stmt->fetch(PDO::FETCH_ASSOC);
			$org_share_t_Cnt = $chk_p_row['share_Cnt'];
			$org_member_Idx = $chk_p_row['member_Idx'];
			$org_share_Cnt = (int)$org_share_t_Cnt - 1;
			//담긴횟수 업데이트
			...
			$insstmt->bindParam(":member_Idx", $org_member_Idx);
			$insstmt->bindparam(":place_Idx",$org_place_Idx);
			$insstmt->bindParam(":share_Cnt", $org_share_Cnt);
			$insstmt->execute();
			$share_del_query = "
				SELECT idx
				...
				WHERE con_Idx = :con_Idx
					AND place_Idx = :place_Idx;
			";
			$share_del_stmt = $DB_con->prepare($share_del_query);
			$share_del_stmt->bindParam(":con_Idx", $conIdx);
			$share_del_stmt->bindParam(":place_Idx", $org_place_Idx);
			$share_del_stmt->execute();
			$share_del_num = $share_del_stmt->rowCount();
			if($share_del_num > 0){
				while($share_del_row = $share_del_stmt->fetch(PDO::FETCH_ASSOC)){
					$share_idx = $share_del_row['idx'];						// 담기고유번호
					$del_share_query = "
						DELETE FROM TB_MEMBERS_SHARE
						WHERE idx = :idx
						LIMIT 1;
					";
					$del_share_stmt = $DB_con->prepare($del_share_query);
					$del_share_stmt->bindParam(":idx", $share_idx);
					$del_share_stmt->execute();
				}
			}
		}else{
			$share_del_query = "
			...
			$share_del_stmt = $DB_con->prepare($share_del_query);
			$share_del_stmt->bindParam(":con_Idx", $conIdx);
			$share_del_stmt->bindParam(":place_Idx", $placeIdx);
			$share_del_stmt->execute();
			$share_del_num = $share_del_stmt->rowCount();
			if($share_del_num > 0){
				while($share_del_row = $share_del_stmt->fetch(PDO::FETCH_ASSOC)){
		...
		if($placeImg != ""){
			...
			if(is_dir($file_dir)){
				function rmdir_all($file_dir) {
				  if (!file_exists($file_dir)) {
					return;
				  }
				  $dhandle = opendir($file_dir);
				  if ($dhandle) {
					while (false !== ($fname = readdir($dhandle))) {
					   if (is_dir( "{$file_dir}/{$fname}" )) {
						  if (($fname != '.') && ($fname != '..')) {
							 $this->rmdir_all("$file_dir/$fname");
						  }
					   } else {
						  unlink("{$file_dir}/{$fname}");
					   }
					}
					closedir($dhandle);
				  }
		...
		if($org_place_Idx != ""){
			$history_del_query = "
				...
				WHERE place_Idx = :place_Idx
					AND con_Idx = :con_Idx
					AND history IN ('지점등록', '담기', '좋아요', '사진업데이트', '댓글등록', '지점공개여부')
					OR idx IN (con_Idx = :con_Idx AND place_Idx = :org_place_Idx AND history = '담기');
			";
			$history_del_stmt = $DB_con->prepare($history_del_query);
			$history_del_stmt->bindParam(":place_Idx", $placeIdx);
			$history_del_stmt->bindParam(":con_Idx", $conIdx);
			$history_del_stmt->bindParam(":org_place_Idx", $org_place_Idx);
			...
			$history_del_query = "
				...
				WHERE place_Idx = :place_Idx
					AND con_Idx = :con_Idx
					AND history IN ('지점등록', '담기', '좋아요', '사진업데이트', '댓글등록', '지점공개여부')
					OR idx IN (con_Idx = :con_Idx AND place_Idx = :place_Idx AND history = '담기');
			";
			$history_del_stmt = $DB_con->prepare($history_del_query);
			$history_del_stmt->bindParam(":place_Idx", $placeIdx);
			$history_del_stmt->bindParam(":con_Idx", $conIdx);
		...
		if($org_place_Idx == ""){	
			$del_p_chk_query = "
				SELECT *
				FROM TB_PLACE
				WHERE org_place_Idx = :org_place_Idx;
			";
			$del_p_chk_stmt = $DB_con->prepare($del_p_chk_query);
			$del_p_chk_stmt->bindParam(":org_place_Idx", $placeIdx);
			$del_p_chk_stmt->execute();
			$del_p_chk_num = $del_p_chk_stmt->rowCount();
			if($del_p_chk_num > 0){
				$del_place_query = "
					UPDATE TB_PLACE
					SET delete_Bit = '1'
	...
	$result = array("result" => "error", "errorMsg" => "이미 삭제한 지점이거나 등록된 지점이 아닙니다.");
}
