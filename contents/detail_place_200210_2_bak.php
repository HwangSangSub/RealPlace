...
if ($place_chk_num > 0) {
	...
	$result = array("result" => "success", "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img, "place_Name" => $place_Name, "memo" => $memo, "tel" => $tel, "day_Bit" => $day_Bit, "otime_Day" => $otime_Day, "week_Bit" => $week_Bit, "otime_Week" => $otime_Week, "img_Cnt" => (string)$img_Cnt, "img" => $img, "like_Cnt" => (string)$like_Cnt, "like_Bit" => $like_Bit, "share_Cnt" => (string)$share_Cnt, "addr" => $addr, "lng" => $lng, "lat" => $lat, "reg_Id" => $reg_Id, "member_Img" => $member_Img, "reg_date" => $reg_date);
    dbClose($DB_con);
    $stmt = null;
} else { //빈값일 경우
