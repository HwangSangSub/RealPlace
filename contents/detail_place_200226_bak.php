...
$place_chk_query = "
	...
	WHERE idx = :idx
		AND delete_Bit = '0'
		AND delete_Bit = '0'
		OR (idx = :idx AND member_Idx = :member_Idx); ";
...
if ($place_chk_num > 0) {
	$query = "
		SELECT con_Idx, org_place_Idx, category, place_Name, place_Icon, memo, smemo, tel, otime_Day, otime_Week, img, like_Cnt, share_Cnt, comment_Cnt, coupon_Cnt, reserv_Bit, coupon_Bit, addr, lng, lat, open_Bit, delete_Bit, reg_Id, reg_date
		FROM TB_PLACE 
		WHERE idx = :idx;
		";
	...
	while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		...
		$org_place_Idx = $row['org_place_Idx'];					// 담아온 지점의 고유번호
		if($org_place_Idx != ""){
			$org_place_query = "
				SELECT con_Idx, reg_Id, mod_Date
				FROM TB_PLACE 
				WHERE idx = :org_place_Idx;
			";
			$org_place_stmt = $DB_con->prepare($org_place_query);
			$org_place_stmt->bindParam(":org_place_Idx", $org_place_Idx);
			$org_place_stmt->execute();
			$org_place_row=$org_place_stmt->fetch(PDO::FETCH_ASSOC);
			$org_con_Idx = $org_place_row['con_Idx'];
			if($org_con_Idx == "" ){
				$org_con_Idx = "";
			}
			$org_con_Name = contentsNameInfo($org_con_Idx);
			if($org_con_Name == "" ){
				$org_con_Name = "";
			}
			$org_reg_Id	= $org_place_row['reg_Id'];
			if($org_reg_Id == "" ){
				$org_reg_Id = "";
			}
			$org_reg_Nname = memNickInfo($org_reg_Id);
			if($org_reg_Nname == "" ){
				$org_reg_Nname = "";
			}
			$org_mod_Date =  $org_place_row['mod_Date'];
			if($org_mod_Date == "" ){
				$org_mod_Date = "";
			}
			if($reg_date != $org_mod_Date){
				$org_upBit = "Y";
			}else{
				$org_upBit = "N";
			}
	...
	if($img_Cnt == "0"){
		$result = array("result" => "success", "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img, "place_Name" => $place_Name, "place_Icon" => $place_Icon, "code_Color" => $code_Color, "memo" => $memo, "tel" => $tel, "day_Bit" => $day_Bit, "otime_Day" => $otime_Day, "week_Bit" => $week_Bit, "otime_Week" => $otime_Week, "img_Cnt" => (string)$img_Cnt, "like_Cnt" => (string)$like_Cnt, "like_Bit" => $like_Bit, "share_Cnt" => (string)$share_Cnt, "coupon_Cnt" => (string)$coupon_Cnt, "coupon_Bit" => $coupon_Bit, "reserv_Bit" => $reserv_Bit, "addr" => $addr, "lng" => $lng, "lat" => $lat, "open_Bit" => $open_Bit, "male_Rate" => (string)(round($mRate)), "female_Rate" => (string)(round($fRate)),"cong_Cnt" => (string)$cong_Cnt, "reg_Id" => $reg_Id, "nick_Name" => $reg_Nname, "member_Img" => $member_Img, "reg_date" => $reg_date, "org_con_Idx" => $org_con_Idx, "org_place_Idx" => $org_place_Idx, "org_con_Name" => $org_con_Name, "org_reg_Id" => $org_reg_Id, "org_reg_Nname" => $org_reg_Nname, "org_delete_Bit" => $org_delete_Bit, "org_mod_Date" => $org_mod_Date, "org_upBit" => $org_upBit;);
	}else{
		$result = array("result" => "success", "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img, "place_Name" => $place_Name, "place_Icon" => $place_Icon, "code_Color" => $code_Color, "memo" => $memo, "tel" => $tel, "day_Bit" => $day_Bit, "otime_Day" => $otime_Day, "week_Bit" => $week_Bit, "otime_Week" => $otime_Week, "img_Cnt" => (string)$img_Cnt, "img" => $img, "like_Cnt" => (string)$like_Cnt, "like_Bit" => $like_Bit, "share_Cnt" => (string)$share_Cnt, "coupon_Cnt" => (string)$coupon_Cnt, "coupon_Bit" => $coupon_Bit, "reserv_Bit" => $reserv_Bit, "addr" => $addr, "lng" => $lng, "lat" => $lat, "open_Bit" => $open_Bit, "male_Rate" => (string)(round($mRate)), "female_Rate" => (string)(round($fRate)),"cong_Cnt" => (string)$cong_Cnt, "reg_Id" => $reg_Id, "nick_Name" => $reg_Nname, "member_Img" => $member_Img, "reg_date" => $reg_date, "org_con_Idx" => $org_con_Idx, "org_place_Idx" => $org_place_Idx, "org_con_Name" => $org_con_Name, "org_reg_Id" => $org_reg_Id, "org_reg_Nname" => $org_reg_Nname, "org_delete_Bit" => $org_delete_Bit, "org_mod_Date" => $org_mod_Date, "org_upBit" => $org_upBit;);
	}
    dbClose($DB_con);
    $stmt = null;
} else { //빈값일 경우
	$result = array("result" => "error", "errorMsg" => "비공개 지점이거나 삭제된 지점입니다.");
}
