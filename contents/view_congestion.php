...
*/
			...
			$query ="
				SELECT A.idx, A.place_Idx, A.lng, A.lat, A.cong_Rate, A.tot_Cnt, female_Cnt, female10, female20, female30 ,female40, female50, male_Cnt, male10, male20, male30 ,male40, male50, A.card_Type1, A.card_Type2
				FROM TB_CONGESTION A
					INNER JOIN TB_PLACE B
						ON A.place_Idx = B.idx
				WHERE B.con_Idx = :con_Idx
			...
			*/
			...
			while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				$card_Type1 = $row['card_Type1'];								// 매출 최고 높은 타입(1: 한식 / 2: 중식 / 3: 양식 / 4: 일식 / 5: 패스트푸드 / 6: 분식)
				$card_Type2 = $row['card_Type2'];								// 매출 두번째 높은 타입(1: 한식 / 2: 중식 / 3: 양식 / 4: 일식 / 5: 패스트푸드 / 6: 분식)
		...
		if($chk_num < 1){
			...
			$summCnt50 = (double)$sum_mCnt50;
			$query ="
				SELECT A.place_Idx, A.idx, A.lng, A.lat, A.cong_Rate, A.tot_Cnt, female_Cnt, female10, female20, female30 ,female40, female50, male_Cnt, male10, male20, male30 ,male40, male50, A.card_Type1, A.card_Type2
				FROM TB_CONGESTION A
					INNER JOIN TB_PLACE B
						ON A.place_Idx = B.idx
				WHERE B.con_Idx = :con_Idx
