			...
			if($sub_Cnt < 1){	//구독지도가 없음.
				...
				if($Num < 1)  { //아닐경우
					...
					while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
						...
						$mresult = ["con_Idx" => $idx, "con_Name" => $con_Name, "con_Lv" => $con_Lv, "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img, "code_Color" => $code_Color, "img_Cnt" => (string)$img_Cnt, "img" => $img, "kml_File" => $kml_File, "memo" => $memo, "pin_Cnt" => (string)$pin_Cnt, "favo_bit" => $favo_bit, "member_Img" => $member_Img, "reg_Id" => $reg_Id, "nick_Name" =>$reg_Nname, "reg_Date" => $regDate];
