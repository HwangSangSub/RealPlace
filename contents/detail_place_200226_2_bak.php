...
if ($place_chk_num > 0) {
	$query = "
		SELECT con_Idx, category, place_Name, place_Icon, memo, smemo, tel, otime_Day, otime_Week, img, like_Cnt, share_Cnt, comment_Cnt, coupon_Cnt, reserv_Bit, coupon_Bit, addr, lng, lat, open_Bit, reg_Id, reg_date
		FROM TB_PLACE 
		WHERE idx = :idx;
		";
	...
	if($img_Cnt == "0"){
		$result = array("result" => "success", "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img, "place_Name" => $place_Name, "place_Icon" => $place_Icon, "code_Color" => $code_Color, "memo" => $memo, "tel" => $tel, "day_Bit" => $day_Bit, "otime_Day" => $otime_Day, "week_Bit" => $week_Bit, "otime_Week" => $otime_Week, "img_Cnt" => (string)$img_Cnt, "like_Cnt" => (string)$like_Cnt, "like_Bit" => $like_Bit, "share_Cnt" => (string)$share_Cnt, "coupon_Cnt" => (string)$coupon_Cnt, "coupon_Bit" => $coupon_Bit, "reserv_Bit" => $reserv_Bit, "addr" => $addr, "lng" => $lng, "lat" => $lat, "open_Bit" => $open_Bit, "male_Rate" => (string)(round($mRate)), "female_Rate" => (string)(round($fRate)),"cong_Cnt" => (string)$cong_Cnt, "reg_Id" => $reg_Id, "nick_Name" => $reg_Nname, "member_Img" => $member_Img, "reg_date" => $reg_date);
	}else{
		$result = array("result" => "success", "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img, "place_Name" => $place_Name, "place_Icon" => $place_Icon, "code_Color" => $code_Color, "memo" => $memo, "tel" => $tel, "day_Bit" => $day_Bit, "otime_Day" => $otime_Day, "week_Bit" => $week_Bit, "otime_Week" => $otime_Week, "img_Cnt" => (string)$img_Cnt, "img" => $img, "like_Cnt" => (string)$like_Cnt, "like_Bit" => $like_Bit, "share_Cnt" => (string)$share_Cnt, "coupon_Cnt" => (string)$coupon_Cnt, "coupon_Bit" => $coupon_Bit, "reserv_Bit" => $reserv_Bit, "addr" => $addr, "lng" => $lng, "lat" => $lat, "open_Bit" => $open_Bit, "male_Rate" => (string)(round($mRate)), "female_Rate" => (string)(round($fRate)),"cong_Cnt" => (string)$cong_Cnt, "reg_Id" => $reg_Id, "nick_Name" => $reg_Nname, "member_Img" => $member_Img, "reg_date" => $reg_date);
	}
    dbClose($DB_con);
    $stmt = null;
} else { //빈값일 경우
