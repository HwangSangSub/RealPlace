	...
	if($mode == "2") {								// 지역순으로 조회할 경우
		if($con_reg_Id != $mem_Id){
			...
			if($con_Cnt != "0"){
				...
				if($p_Cnt == 1){
					...
					$open_Bit = $row['open_Bit'];							// 공개여부 (0: 공개, 1: 비공개)
					$mresult = ["place_Idx" => $idx, "category" => $category, "category_Name" => $category_Name, "place_Name" => $place_Name, "share_Cnt" => (string)$share_Cnt, "lng" => $lng, "lat" => $lat, "si" => $si, "addr" => $address, "open_Bit" => (string)$open_Bit];
		...
		if($con_reg_Id != $mem_Id){
			...
			if($con_reg_Id != $mem_Id){
				// 지점정보
				$query = "
					SELECT idx, place_Name, lng, lat, category, share_Cnt, ( 6371 * acos( cos( radians(:locatLat) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians(:locatLng) ) + sin( radians(:locatLat) ) * sin( radians( lat ) ) ) ) AS  distance, open_Bit
					FROM TB_PLACE 
					WHERE con_Idx = :con_Idx
						AND open_Bit = '0'
						AND delete_Bit = '0'
			...
			if($p_Cnt == 1){
				...
				$distance = round($distance, 1);						// 거리 소수첫째 자리까지 반올림
				$open_Bit = $row['open_Bit'];							// 공개여부(0: 공개, 1: 비공개)
				$mresult = ["place_Idx" => $idx, "category" => $category, "category_Name" => $category_Name, "place_Name" => $place_Name, "share_Cnt" => (string)$share_Cnt, "lng" => $lng, "lat" => $lat, "distance" => (string)$distance, "open_Bit" => (string)$open_Bit];
				 array_push($data, $mresult);
				...
				$result = array("result" => "success", "pin_Cnt" => ["totCnt" => "0", "page" => "0"], "lists" =>["place_Idx" => "", "category" => "", "category_Name" => "", "place_Name" => "", "share_Cnt" => "", "lng" => "", "lat" => "", "distance" => "0", "open_Bit" => "0"]);
				echo json_encode($result, JSON_UNESCAPED_UNICODE); 
			}
		}
	}
