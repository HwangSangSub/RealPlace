...
$listPlaceCnt = (int)$list_PlaceCnt - 1;
		...
		if($search != ""){	//검색어가 있을 경우 검색값 추가  
			...
			$cntQuery = "
				...
				WHERE ".$category."
					...
					AND a.open_Bit = '0'
					AND a.admin_Bit = '0'
					AND (SELECT count(idx) FROM TB_PLACE WHERE con_Idx = a.idx AND open_Bit = '0' AND delete_Bit = '0' OR idx in (SELECT place_Idx  FROM TB_MEMBERS_SHARE WHERE con_Idx = a.idx AND use_Bit = 'Y')) > ".$listPlaceCnt."
				GROUP BY a.idx
						...
						while($img_row2=$img_stmt2->fetch(PDO::FETCH_ASSOC)){
							...
							if(is_dir($m_file2)){
								...
								foreach ($files2 as $f2) {
									$img_FileName2 = "http://places.gachita.co.kr/contents/place_img/".$pimg2."/".$f2;
