	...
	if($area_Code != ""){
		...
		if($chk_num < 1){
			...
			while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				$mresult = ["idx" => (int)$idx, "cong_Rate" => (int)$cong_Rate, "tot_Cnt" => (double)$tot_Cnt, "fRate" => round($fRate), "mRate" => round($mRate), "age10" => $age10, "age20" => $age20, "age30" => $age30, "age40" => $age40, "age50" => $age50, "rate_10" => round($rate_10), "rate_20" => round($rate_20), "rate_30" => round($rate_30), "rate_40" => round($rate_40), "rate_50" => round($rate_50)];
				$geojson = array(
					"geometry" => array(
						"coordinates" => $poiresult,
						"type" => "Point"
					),
					"properties" => $mresult,
					"type" => "Feature"
			...
			$chkData['timestamp1'] = date(DATE_W3C, time());
			$chkData['timestamp2'] = date(DATE_RFC3339, time());
			$chkData['timestamp3'] = date(DATE_RSS, time());
			$chkData['timestamp4'] = date(DATE_ATOM,mktime(0,0,0,8,15,1947));
			$chkData['timestamp5'] = (String)$date;
			//$chkData['type'] = "FeatureCollection";
			$output = str_replace('\\\/', '/', json_encode($chkData));
			echo $output;
		...
		if($chk_num < 1){
			...
			$chkData = [];
			$chkData['features'] = $data;
			//$chkData['timestamp'] = date(c,strtotime(DU_TIME_YMDHIS));
			$chkData['type'] = "FeatureCollection";
