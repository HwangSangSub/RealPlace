...
if ($reg_Id != "") {
	...
	$like_cnt_stmt->bindParam(":reg_Id", $reg_Id);
	...
	if($mode == "1"){							//내 활동 만
		$query = "
			SELECT idx, con_Idx, place_Idx, mem_Id, history, reg_Date
			FROM TB_HISTORY 
			WHERE reg_Id = :reg_Id
				AND history NOT IN ( '담기', '신고')
		...
		$like_stmt->bindParam(":reg_Id", $reg_Id);
		$like_stmt->execute();
		$like_Id = "";
		$int = 1;
    ...
    if($chknum < 1)  { // 등록된 히스토리가 없는 경우
		...
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
			...
			$history_con = historyInfo($idx, $reg_Id, $history);								// 히스토리정리
			if($history_con == ''){
				$history_con ="";
			}
			...
			$result = ["mem_Id" => $mem_Id, "member_Img" => $member_Img, "con_Idx" => $con_Idx, "con_Name" => $con_Name, "place_Idx" => $place_Idx, "place_Name" => $place_Name, "history" => $history_con, "reg_Date" => $reg_Date];
