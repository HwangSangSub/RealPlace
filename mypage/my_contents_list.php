...
if (memChk($reg_Id) == "1") {
	...
	if($mem_Id != $reg_Id){
		...
		$cntQuery = "
			...
			//                 AND (SELECT count(idx) FROM TB_PLACE WHERE con_Idx = a.idx AND open_Bit = '0' AND delete_Bit = '0' OR idx in (SELECT place_Idx  FROM TB_MEMBERS_SHARE WHERE con_Idx = a.idx AND member_Idx = :member_Idx AND use_Bit = 'Y')) > 4 다른사람이 프로필 볼때 등록된 지점이 4개 이하면 숨김처리
		...
		//지도 기본테이블 조회
		$query = "
				SELECT a.idx, a.con_Name, a.con_Lv, a.category, a.img, a.tag, a.like_Cnt, a.kml_File, a.memo, a.reg_Id, a.reg_Date, a.end_Date, a.open_Bit, a. map_Img
				FROM TB_CONTENTS a
					LEFT OUTER JOIN TB_PLACE b ON a.idx = b.con_Idx OR b.idx in (SELECT place_Idx  FROM TB_MEMBERS_SHARE WHERE con_Idx = a.idx AND member_Idx = :member_Idx AND use_Bit = 'Y')
				WHERE a.reg_Id = :reg_Id
					AND a.member_Idx = :member_Idx
		...
		if($Num < 1)  { //아닐경우
			...
			while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				if($mem_Id != 'GUEST'){
					...
					$subs_stmt->bindParam(":member_Idx", $memberIdx);
					$subs_stmt->execute();
				...
				if($img_Name == ""){											// 이미지가 없을 경우 
					...
					if($locat_Img == ""){
						if($map_Img == "" ){
							...
							$locat_Img = "http://places.gachita.co.kr/contents/map_img/contents/".$map_Img;
						}
					}
		...
		//지도 기본테이블 조회
		$query = "
			SELECT idx, con_Name, con_Lv, category, img, tag, like_Cnt, kml_File, memo, reg_Id, reg_Date, end_Date, a.open_Bit, a.map_Img
			FROM TB_CONTENTS as a 
			WHERE reg_Id = :reg_Id
				AND member_Idx = :member_Idx
				AND a.delete_Bit = '0'
