...
if ($mem_Id != "" || $mem_Id != "GUEST") {
	...
	if($Num < 1)  { //아닐경우
		...
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			$mresult = ["con_Idx" => $idx, "con_Name" => $con_Name, "con_Lv" => $con_Lv, "category" => $category, "code_Name" => $code_Name, "code_Img" => $code_Img,  "img_Cnt" => (string)$img_Cnt, "img" => $img, "tag" => $tag, "tag_Cnt" => (string)$tag_Cnt, "kml_File" => $kml_File, "memo" => $memo, "pin_Cnt" => (string)$pin_Cnt, "subs_bit" => $subs_bit, "reg_Id" => $reg_Id];
